...
import Tailwind.Traders.Stock.Api.config.CosmosDbConfig;
import Tailwind.Traders.Stock.Api.models.StockItem;

@Service("AZURE")
public class AZUREStockItemRepository implements StockItemRepository {
	...
	public Integer count() {
		...
		CosmosContainer cosmosContainer = cosmosClient.getDatabase(getTodoDatabase().getId())
				.getContainer(getTodoCollection().getId());
		Iterator<StockItem> allItems = cosmosContainer.readAllItems(new PartitionKey(partitonkey), StockItem.class)
				.iterator();
	...
	private CosmosContainerProperties getTodoCollection() {
		...
		Iterator<CosmosContainerProperties> collectionList = cosmosDatabase
				.queryContainers("SELECT * FROM root r WHERE r.id='" + COLLECTION_ID + "'", null).iterator();

		if (collectionList.hasNext()) {
			containerCache = collectionList.next();
		} else {
			try {
				CosmosContainerProperties cosmosContainerProperties = new CosmosContainerProperties(COLLECTION_ID,
						"/partition");

				containerCache = cosmosDatabase.createContainer(cosmosContainerProperties).getProperties();
			} catch (CosmosException e) {
				e.printStackTrace();
			}
		}
